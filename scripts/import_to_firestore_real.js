/**
 * GRAN ARK 顧客データFirestoreインポートスクリプト（実環境版）
 * CSVデータから実際のFirestoreにデータをインポート
 */
...
async function importToFirestore(customers) {
  ...
  for (let i = 0; i < customers.length; i += batchSize) {
    ...
    for (const customer of batchCustomers) {
      try {
        ...
        docIds.push({ customerId: customer.customer_id, docId: docRef.id });
        
        // 進捗表示
        const currentIndex = customers.indexOf(customer) + 1;
        const progress = Math.floor(currentIndex / customers.length * 100);
        const progressBar = '█'.repeat(Math.floor(progress / 2)) + '░'.repeat(50 - Math.floor(progress / 2));
        
        process.stdout.write(`\r[${progressBar}] ${progress}% (${currentIndex}/${customers.length})`);
    ...
    try {
      await batch.commit();
      successCount += batchCustomers.length;
    } catch (error) {
      console.error('\nバッチコミットエラー:', error);
      errorCount += batchCustomers.length;
    }
  }
  ...
  // 最初の5件のドキュメントIDを表示
  console.log('\n📝 インポートされたドキュメント（最初の5件）:');
  docIds.slice(0, 5).forEach(doc => {
    console.log(`   顧客ID: ${doc.customerId} → Firestore ID: ${doc.docId}`);
  });
